...
async function getCachedOrFetchProducts(location: string): Promise<Record<string, unknown>[]> {
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_DURATION_MS) {
    console.log(`[Location API SD] ✓ Cache HIT for ${location} (${cached.products.length} products)`);
  ...
  try {
    ...
    if (apiKey) {
      ...
      if (!requestApiKey || requestApiKey !== apiKey) {
        console.warn(`[Location API SD] Unauthorized access attempt from ${request.headers.get("x-forwarded-for") || "unknown"}`);
    ...
    console.log(`[Location API SD] Fetched ${products.length} products, ${discountCount} with active discounts`);
